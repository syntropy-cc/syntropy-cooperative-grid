...
func TestConfigE2E_ConfigurationGeneration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tempDir := helpers.CreateTempDir(t, "config_generation_e2e_test")
...
// TestConfigE2E_StructureCreation testa a criação de estrutura end-to-end
func TestConfigE2E_StructureCreation(t *testing.T) {
	tests := []struct {
		name    string
		wantErr bool
	}{
		{
			name:    "should create complete structure end-to-end successfully",
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_structure_e2e_test")
			configDir := filepath.Join(tempDir, "config")
			os.MkdirAll(configDir, 0755)
			...
			// Executar criação de estrutura completa
			err := configurator.CreateStructure()
			if (err != nil) != tt.wantErr {
...
// TestConfigE2E_KeyGeneration testa a geração de chaves end-to-end
func TestConfigE2E_KeyGeneration(t *testing.T) {
	tests := []struct {
		name    string
		wantErr bool
	}{
		{
			name:    "should generate keys end-to-end successfully",
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_keys_e2e_test")
...
// TestConfigE2E_ConfigurationValidation testa a validação de configuração end-to-end
func TestConfigE2E_ConfigurationValidation(t *testing.T) {
	tests := []struct {
		name      string
		...
		{
			name: "should validate valid configuration end-to-end",
...
`
		...
		{
			name: "should fail validation for invalid configuration end-to-end",
			setupFunc: func(configDir string) error {
		...
		{
			name: "should fail validation for missing configuration end-to-end",
			setupFunc: func(configDir string) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_validation_e2e_test")
			configDir := filepath.Join(tempDir, "config")
			os.MkdirAll(configDir, 0755)
...
// TestConfigE2E_ConfigurationBackup testa o backup de configuração end-to-end
func TestConfigE2E_ConfigurationBackup(t *testing.T) {
	tests := []struct {
		name       string
		backupName string
		setupFunc  func(string) error
		...
		{
			name:       "should backup configuration end-to-end successfully",
...
`
		...
		{
			name:       "should handle empty backup name end-to-end",
			backupName: "",
			setupFunc: func(configDir string) error {
				// Criar arquivo de configuração
				configPath := filepath.Join(configDir, "manager.yaml")
...
`
		...
		{
			name:       "should fail backup for missing configuration end-to-end",
			backupName: "config_backup",
			setupFunc: func(configDir string) error {
				// Não criar arquivo de configuração
				return nil
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_backup_e2e_test")
			configDir := filepath.Join(tempDir, "config")
...
// TestConfigE2E_ConfigurationRestore testa a restauração de configuração end-to-end
func TestConfigE2E_ConfigurationRestore(t *testing.T) {
	tests := []struct {
		name       string
		backupPath string
		setupFunc  func(string) error
		...
		{
			name:       "should restore configuration end-to-end successfully",
...
`
		...
		{
			name:       "should fail restore for missing backup end-to-end",
		...
		{
			name:       "should fail restore for invalid backup end-to-end",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_restore_e2e_test")
			configDir := filepath.Join(tempDir, "config")
			backupDir := filepath.Join(tempDir, "backups")
			os.MkdirAll(configDir, 0755)
			os.MkdirAll(backupDir, 0755)
...
// TestConfigE2E_ErrorHandling testa o tratamento de erros end-to-end
func TestConfigE2E_ErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "should handle nil options end-to-end",
			options: nil,
			wantErr: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Criar diretório temporário para testes
			tempDir := helpers.CreateTempDir(t, "config_error_e2e_test")
			configDir := filepath.Join(tempDir, "config")
			os.MkdirAll(configDir, 0755)
...
// TestConfigE2E_Concurrency testa concorrência end-to-end
func TestConfigE2E_Concurrency(t *testing.T) {
	t.Run("should handle concurrent configuration operations end-to-end", func(t *testing.T) {
		// Criar diretório temporário para testes
		tempDir := helpers.CreateTempDir(t, "config_concurrent_e2e_test")
		configDir := filepath.Join(tempDir, "config")
		os.MkdirAll(configDir, 0755)
...
// TestConfigE2E_Performance testa performance end-to-end
func TestConfigE2E_Performance(t *testing.T) {
	t.Run("should complete configuration operations within reasonable time end-to-end", func(t *testing.T) {
		// Criar diretório temporário para testes
		tempDir := helpers.CreateTempDir(t, "config_perf_e2e_test")
		configDir := filepath.Join(tempDir, "config")
		os.MkdirAll(configDir, 0755)
